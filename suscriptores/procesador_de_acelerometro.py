...
class ProcesadorAcelerometro:
    ...
    def consume(self):
        try:
            ...
            channel.basic_consume(self.callback, queue='SensorAcelerometro')
    ...
    def callback(self, ch, method, properties, body):
        ...
        x=float(json_message['SensorAcelerometro_x'])
        y=float(json_message['SensorAcelerometro_y'])
        z=float(json_message['SensorAcelerometro_z'])
        if (z < -9) and  ( (y<-9  or y>1) or (x<-1 or x>9)):
            print x,"_",y,"_",z
            monitor = Monitor()
            monitor.print_notification('Esta en el suelo',json_message['datetime'], json_message['id'])
        time.sleep(1)
